...
class HttpClient {
                ...
                dataTask = session.dataTask(with: request) { [weak self] (data, response, error) in
                    ...
                    if let error = error {
                        if error.localizedDescription == "cancelled" {
                            return // We dont need to alert the user of this. This is when a task is cancelled.
                        }
    ...
    private func checkResponse(response: URLResponse?, data: Data?) -> Result<Data, APIErrors> {
        ...
        if (200..<300) ~=  urlResponse.statusCode {
            ...
            guard let data = data else { return .failure(APIErrors.validationError("Unable to retrieve data.")) }
            return .success(data)
        }
        else {
            guard let data = data else { return .failure(APIErrors.validationError("Unable to retrieve data.")) }
            ...
            do {
                ...
                return .failure(APIErrors.validationError("Failed with status code:  \(urlResponse.statusCode). Unable to decode data."))
            }
        }
        return .failure(APIErrors.invalidResponseError)
    }
